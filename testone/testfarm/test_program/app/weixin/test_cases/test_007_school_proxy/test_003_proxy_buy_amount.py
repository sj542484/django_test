...
from testfarm.test_program.app.weixin.test_data.phone_list import amount_phone_error,amount_phone_right
    ...
    def test_student_bill_error(self):
        ...
        if self.statement_page.wait_check_page():
            ...
            if self.school_proxy.wait_buy_page():
                ...
                self.school_proxy.click_this()
                if self.school_proxy.wait_amount_input_page():
                    self.school_proxy.click_next()
                    self.school_proxy.input_amount_phone(amount_phone=amount_phone_error)
                    self.school_proxy.click_next()
                    self.school_proxy.phone_error_tips()
                    self.school_proxy.click_warning()
                    self.school_proxy.get_warnning_text()
                    self.school_proxy.click_quit()
                    self.school_proxy.input_amount_phone(amount_phone=amount_phone_right,content=(' '.join(amount_phone_error)).replace('\n',''))
                    self.school_proxy.click_next()
                    if self.school_proxy.wait_choose_stu_page():
                        self.school_proxy.click_next()
                        self.school_proxy.judge_tips_have_no_stu()
                        ...
                        if self.school_proxy.wait_choose_card_page():
                            ...
                            self.public.school_info(after_info=after_info,before_info=before_info,peo_num=peo_num,unit_price=unit_price)
                            self.quit()
                        else:
                            print('❌error：进入页面错误')
                    else:
